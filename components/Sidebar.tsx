...
const Sidebar = ({ reponsponsiveProps, sidebarProps }: any) => {
  const Tab = ({ icon, desc, title, props }: any) => {
    return (
      <li
        className={`overflow-hidden mx-3 md:group-hover:mr-0 duration-200 transition-all`}
      >
        <Link
          className="py-1 my-2 flex justify-start items-center gap-7 md:px-2.5 hover:bg-gradient-to-r from-white/10 to-transparent group-hover:sm:pr-20 sm:pr-0 pr-16 sm:px-0 transition-all rounded-l-md"
          href=""
        >
          <Image
            src={icon}
